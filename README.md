...
- `I have three environment facts to tell you: the workspace root is E:\VibeCoding, use uv for Python, and do not modify D:\Shared.`
- `Before you continue, record two local rules: do not edit .env files and do not touch the deployment scripts.`
- `Please collect my environment, and note that C:\Secrets and config\prod.yaml are protected.`
...
- `我有三项环境信息要告知：工作区根目录是 E:\VibeCoding，Python 一律用 uv，不要修改 D:\Shared。`
- `继续之前先记录两条规则：不要编辑 .env 文件，不要动部署脚本。`
- `请采集本机环境，并记住 C:\Secrets 和 config\prod.yaml 是受保护目标。`
